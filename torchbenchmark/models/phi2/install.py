...
def convert_hf_checkpoint(
    *,
    checkpoint_dir: Path = Path("torchbenchmark/models/phi2/.data/checkpoints/microsoft/phi-2"),
...
def hf_download(repo_id: Optional[str] = None, hf_token: Optional[str] = None) -> None:
    ...
    os.makedirs(f"torchbencmark/models/phi2/.data/checkpoints/{repo_id}", exist_ok=True)
    try:
        snapshot_download(repo_id, local_dir=f"torchbenchmark/models/phi2/.data/checkpoints/{repo_id}", local_dir_use_symlinks=False, token=hf_token)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--repo_id', type=str, default="microsoft/phi-2", help='Repository ID to download from.')
